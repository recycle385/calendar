...
export function FeatureGrid() {
  return (
    <section className="py-16 max-lg:py-12 max-md:py-[38px]" id="features">
      <div
        className={`${shellClass} grid grid-cols-[330px_minmax(0,1fr)] items-center gap-16 max-lg:grid-cols-1 max-lg:gap-[34px] max-md:gap-[26px]`}
      >
        <header className="max-md:text-left">
          ...
          <h2 className="mt-3 mb-0 text-[32px] leading-[1.3] font-black tracking-[-0.055em] text-ink-900 max-md:text-[clamp(27px,7.2vw,34px)]">
            <span className="max-md:hidden">모임이 쉬워지는 이유</span>
        ...
        <div className="grid grid-cols-3 max-md:grid-cols-2 max-md:gap-3 max-[480px]:gap-2.5">
          {features.map(({ icon: Icon, title, text }, index) => (
            <article
              className={`flex min-h-28 items-start gap-4 px-[26px] pb-[27px] text-left max-md:min-h-[190px] max-md:flex-col max-md:items-center max-md:justify-start max-md:gap-[15px] max-md:rounded-[18px] max-md:border max-md:border-[#e4ebf4] max-md:bg-white max-md:px-3.5 max-md:py-[18px] max-md:text-center max-[480px]:min-h-[184px] max-[480px]:rounded-[17px] max-[480px]:px-2 ${index % 3 === 0 ? "pl-0 max-md:pl-3.5 max-[480px]:pl-2" : "border-l border-[#e4ebf4] max-md:border-l-[#e4ebf4]"} ${index < 3 ? "border-b border-[#e4ebf4]" : "pt-[27px] pb-0"} ${index >= 4 ? "max-md:hidden" : ""}`}
              key={title}
            >
              ...
              <div>
                <h3 className="mt-1 mb-[7px] text-[15px] leading-[1.35] font-black text-ink-900 max-md:m-0 max-md:mb-[7px] max-md:text-lg max-[480px]:text-[17px]">
                  {title}
                </h3>
                <p className="m-0 text-sm leading-[1.65] text-[#71839d] [word-break:keep-all] max-[480px]:text-[13px]">
                  {text}
                </p>
              </div>
            </article>
