...
import { VoteAnalysisPanel } from "../../../../domains/analysis";
...
export function CalendarDetailPage() {
  ...
  const requestedDetailTab: DetailTab =
    requestedTab && DETAIL_TABS.includes(requestedTab as DetailTab)
  ...
  const activeParticipantSession = reconciliation.isRequired
    ? null
    : participantSession;

  const calendarQuery = useQuery({
    ...calendarDetailQuery(slug),
    enabled: Boolean(slug && activeParticipantSession),
  });
  const participants = useQuery({
    ...participantsQuery(slug),
    enabled: Boolean(slug && activeParticipantSession),
  });
  const voteStatus = useQuery({
    ...voteStatusQuery(slug),
    enabled: Boolean(slug && activeParticipantSession),
  });
  ...
  if (memberIdentityUnavailable) {
    const message =
      authStatus === "restore-failed"
        ? "네트워크 문제로 회원과 참여 세션을 확인하지 못했어요. 새로고침 후 다시 시도해주세요."
        : "회원과 참여 세션을 확인하고 있어요.";
    return (
      <WorkspaceLayout>
        <section className="grid min-h-[250px] place-content-center text-center text-[#69809f]">
          {message}
        </section>
      </WorkspaceLayout>
    );
  ...
  if (reconciliation.isRequired) {
    return (
      <WorkspaceLayout>
        <section
          className={`${panelClass} grid min-h-[320px] place-content-center text-center text-[16px] text-[#69809f]`}
        >
    ...
    return (
      <WorkspaceLayout>
        <section className="grid min-h-[250px] place-content-center justify-items-center gap-3 text-center text-[#69809f]">
          <h1 className="m-0 text-[22px] font-black text-[#19345d]">
            삭제된 캘린더예요.
          </h1>
          <p className="m-0 leading-[1.65]">
            방장이 캘린더를 삭제해서 더 이상 참여할 수 없어요.
          </p>
          <Link className={`${buttonClass} ${primaryButtonClass}`} to="/">
            홈으로 돌아가기
          </Link>
        </section>
      </WorkspaceLayout>
  ...
  const liveCalendar =
    calendar && realtime.isClosed ? { ...calendar, is_closed: true } : calendar;
  ...
  const isVotingClosed = Boolean(
    liveCalendar &&
    (liveCalendar.is_closed ||
      getDaysUntilCalendarDate(liveCalendar.vote_end_date) < 0),
  );
  const tab: DetailTab = isVotingClosed
    ...
    : requestedDetailTab === "settings" && !isHost
      ...
      : requestedDetailTab;
  const shareUrl =
    (location.state as DetailLocationState | null)?.shareUrl ??
    `${window.location.origin}/c/${slug}/join`;
  const changeTab = (nextTab: DetailTab) =>
    setSearchParams(nextTab === "vote" ? {} : { tab: nextTab });
  ...
  return (
        ...
        tab === "vote" ? (
          <VoteDetailAside
            ...
            analysisContent={
              <VoteAnalysisPanel
                slug={slug}
                participantToken={activeParticipantSession!.participantToken}
                embedded
              />
            }
          />
          ...
          <>
            <DetailAside
              ...
              showRecommendations={tab !== "status"}
            />
    ...
    >
      <Link
        className="my-[7px] mb-3.5 inline-flex items-center gap-[3px] text-[13px] font-extrabold text-[#5c83ba]"
        to="/calendars"
      >
      ...
      {calendarQuery.isPending ? (
        ...
        <section
          className={`${panelClass} grid min-h-[250px] place-content-center justify-items-center gap-3 text-center text-[#69809f]`}
        >
          <h1 className="m-0 text-[22px] font-black text-[#19345d]">
            캘린더 정보를 불러오지 못했어요.
          </h1>
        ...
        <>
          ...
          <div
            className={
              isVotingClosed
                ? "mt-4"
                : "mt-4 grid grid-cols-[150px_minmax(0,1fr)] gap-4 max-[800px]:grid-cols-1"
            }
          >
            {!isVotingClosed && (
              <DetailTabRail
            ...
            <section className="min-w-0">
              {tab === "vote" && (
                <VotePanel
                  ...
                  selectedDate={voteDateSelection.selectedDate}
                  loading={
                    (!voteStatus.data && voteStatus.isPending) ||
                    (!ownVotes.data && ownVotes.isPending)
                  }
                  loadError={Boolean(
                    (voteStatus.isError && !voteStatus.data) ||
                    (ownVotes.isError && !ownVotes.data),
                  )}
                  refetchError={Boolean(
                    (voteStatus.isRefetchError && voteStatus.data) ||
                    (ownVotes.isRefetchError && ownVotes.data),
                  )}
