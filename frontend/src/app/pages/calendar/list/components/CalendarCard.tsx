...
export function CalendarCard({ calendar }: { calendar: Calendar }) {
  return (
    <Link className="relative grid min-h-[175px] grid-cols-[140px_minmax(0,1fr)] overflow-hidden rounded-[14px] border border-[#e1eaf5] bg-white transition duration-200 hover:-translate-y-0.5 hover:border-[#bcd8ff] hover:shadow-[0_12px_24px_rgba(30,100,192,0.1)] max-[520px]:grid-cols-[105px_minmax(0,1fr)]" to={`/c/${calendar.slug}`}>
      ...
      <div className="min-w-0 px-4 pt-[18px] pr-7 pb-3.5 max-[520px]:px-[13px] max-[520px]:pt-3.5 max-[520px]:pr-6 max-[520px]:pb-3">
        ...
        <div className="mt-[13px] grid gap-[5px] text-xs font-bold text-[#6981a1] [&>span]:flex [&>span]:items-center [&>span]:gap-[5px]">
          <span><CalendarDays size={15} /> {formatDate(calendar.start_date)} — {formatDate(calendar.end_date)}</span>
          <span className="max-[520px]:hidden"><Users size={15} /> 참여 현황 보기</span>
        </div>
      </div>
