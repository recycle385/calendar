...
describe('게스트 캘린더 참여', () => {
  ...
  it('등록되지 않은 게스트면 신규 참가자로 등록한다', async () => {
    vi.mocked(loginParticipant).mockRejectedValue(new ApiError(401, {
      success: false,
    ...
    expect(registerParticipant).toHaveBeenCalledWith('calendar', {
      nickname: '게스트',
      password: 'password',
    })
  ...
  it('기존 닉네임의 비밀번호가 틀리면 최초 인증 오류를 유지한다', async () => {
    const loginError = new ApiError(401, {
      success: false,
      message: '닉네임 또는 비밀번호가 일치하지 않습니다',
    })
    vi.mocked(loginParticipant).mockRejectedValue(loginError)
    vi.mocked(registerParticipant).mockRejectedValue(new ApiError(409, {
      success: false,
  ...
  it('인증 실패가 아닌 오류에는 신규 등록을 시도하지 않는다', async () => {
