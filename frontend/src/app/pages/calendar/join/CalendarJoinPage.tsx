...
export function CalendarJoinPage() {
  ...
  const [mode, setMode] = useState<JoinMode>(
    status === "authenticated" ? "account" : "guest",
  );
  const linkedEntryStarted = useRef(false);
  const form = useForm<JoinForm>({
    resolver: zodResolver(joinSchema),
    defaultValues: { nickname: user?.nickname ?? "", password: "" },
  });
  ...
  const joinedCalendars = useQuery({
    ...
    enabled: Boolean(
      slug && status === "authenticated" && userUuid && accessToken,
    ),
  });
  ...
  const joinError = useMemo(() => {
    ...
    if (isApiError(joinMutation.error) && joinMutation.error.status === 401)
      return "닉네임이 사용 중이거나 비밀번호가 달라요. 다시 확인해주세요.";
    if (isApiError(joinMutation.error) && joinMutation.error.status === 409)
      return "이미 사용 중인 닉네임이에요. 다른 닉네임을 입력해주세요.";
    return "참여 처리에 실패했어요. 네트워크를 확인한 뒤 다시 시도해주세요.";
  }, [joinMutation.error]);

  const linkedParticipation = joinedCalendars.data?.calendars.find(
    (calendar) => calendar.slug === slug,
  );

  useEffect(() => {
    if (
      !linkedParticipation ||
      usableExistingSession ||
      linkedEntryStarted.current
    )
      return;
    ...
    return (
      <WorkspaceLayout>
        <section className="grid min-h-[250px] place-content-center justify-items-center gap-3 text-center text-[#69809f]">
          ...
          <p>받은 링크를 다시 확인해주세요.</p>
          <Link className={`${buttonClass} ${secondaryButtonClass}`} to="/">
            홈으로 돌아가기
          </Link>
        </section>
      </WorkspaceLayout>
    );
  if (status === "authenticated" && joinedCalendars.isPending)
    return (
      <WorkspaceLayout>
        <section className="grid min-h-[250px] place-content-center text-center text-[#69809f]">
          참여 이력을 확인하고 있어요.
  ...
  if (status === "authenticated" && joinedCalendars.isError)
    return (
      <WorkspaceLayout>
        <section className="grid min-h-[250px] place-content-center justify-items-center gap-3 text-center text-[#69809f]">
          <p>참여한 캘린더 정보를 확인하지 못했어요.</p>
          <button
            className={`${buttonClass} ${secondaryButtonClass}`}
            type="button"
            onClick={() => void joinedCalendars.refetch()}
          >
            다시 시도
          </button>
        </section>
