...
let mainRefreshInFlight: {
  session: MainAuthSessionSnapshot;
...
function isSameMainSession(
  left: MainAuthSessionSnapshot,
  right: MainAuthSessionSnapshot,
) {
  return (
...
async function refreshMainAccessToken(requestSession: MainAuthSessionSnapshot) {
  if (!mainAuthController)
    throw new Error("Main 인증 갱신 핸들러가 설정되지 않았습니다.");
  if (
    !mainRefreshInFlight ||
    !isSameMainSession(mainRefreshInFlight.session, requestSession)
  ) {
    ...
    const promise = controller
      ...
      .finally(() => {
        if (mainRefreshInFlight?.promise === promise)
...
export async function apiRequest<T>(
  ...
  const requestSession =
    options.auth === "main" && mainAuthController
...
async function executeRequest<T>(
  ...
  if (!response.ok) {
    ...
    if (
      ...
      if (
        !requestSession ||
        !isSameMainSession(requestSession, currentSession)
      ) {
